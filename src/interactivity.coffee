...
({ P, compareNumber, styleZoom }, {Tube, createTubes}, {FilterSearch}, {History}, {CentralStationEmbedder}) ->
	...
	class View
		...
		update: (graph) ->
			...
			color_radicals = []
			edge.each (d) ->
				radical = d.line.data.radical
				color_radicals.push radical if radical not in color_radicals
				d3.select(@).classed "line_"+radical, true
			for rad, rad_i in color_radicals
				selector = ".line_" + rad
				d3.selectAll(selector).style("stroke", colors[rad_i])
